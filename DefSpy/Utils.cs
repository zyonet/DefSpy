...
/// <summary>
/// credit code is taken from https://github.com/icsharpcode/ILSpy/blob/master/ILSpy.AddIn/Utils.cs
/// </summary>
namespace ICSharpCode.ILSpy.AddIn
{
	...
	static class Utils
	{
		...
		public static List<Tuple<string, object> >GetAllProperties(Properties properties)
		{
			var result = new List<Tuple<string, object>>();
			for (int i = 0; i < properties.Count; i++) {
				try
                {
                    var p = properties.Item(i) as Property;
                    if (p != null)
                    {
                        result.Add(new Tuple<string, object>(p.Name, p.Value));
                    }
                } catch {
					continue;
				}
